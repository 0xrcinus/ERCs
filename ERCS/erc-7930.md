...
## Abstract
This proposal introduces a **binary format** to describe a chain specific address. Additionally, it defines standardized encodings of that identifier: a text encoding suitable for display and copy-paste, and a JSON encoding suitable for programmatic use.
...
This proposal builds on insights from [CAIP-10] and [CAIP-50]. It introduces a canonical binary Interoperable Address format, and defines standardized encodings of that address — the Interoperable Address String and Interoperable Address JSON — for use in various contexts. This format:
...
- Provides standardized encodings for both human-readable and programmatic use cases.
...
### Terminology
**Target Address**
: The address itself, independent of chain context. Serialized per the [CAIP-350] rules for the applicable namespace. In the context of the _Interoperable Address String_, _Interoperable Address JSON_, and _Interoperable Address_ examples below, the target address is `0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045`.
...
: A text encoding of an _Interoperable Address_ for display and copy-paste. Format: `<address> @ <chain>`. e.g. `0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045@eip155:1`

**Interoperable Address JSON**
: A JSON encoding of an _Interoperable Address_ for programmatic use. Format: `{"address": "...", "chain": "..."}`. e.g. `{"address": "0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045", "chain": "eip155:1"}` 
...
Both the _Interoperable Address String_ and _Interoperable Address JSON_ are structured encodings of an underlying _Interoperable Address_, derived from the binary form. They both expose the same logical components (`address` and `chain`) and use the same encoding/decoding logic via the relevant [CAIP-350] profile. They differ only in representation format: the String uses a text format suitable for display and copy-paste, while the JSON uses a structured object suitable for programmatic use.

#### Components

Both encodings expose two logical components:

- **`address`**: The _target address_, serialized according to the normalization and serialization rules defined in the relevant [CAIP-350] profile. If you are targeting all addresses within a CASA namespace (i.e., when `AddressLength` is zero), this component MUST be omitted or empty.

- **`chain`**: The string representation of a specific blockchain as defined in [CAIP-350]. For example `eip155:1` for Ethereum Mainnet, or `solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpKuc147dw2N9d` for Solana Mainnet. If you are representing a specific address namespace without intending to refer to a particular chain (i.e., when `ChainReferenceLength` is zero), use the CASA namespace alone, with no trailing colon - for example, `eip155` or `solana`. It is not possible to specify a reference without a namespace.

#### Interoperable Address String Format

The _Interoperable Address String_ format is `<address> @ <chain>` where the components match the following regular expressions:
...
Example: `0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045@eip155:1`

#### Interoperable Address JSON Format

The _Interoperable Address JSON_ canonical object form is:
...
Both encodings are thin, reversible views over the same canonical binary Interoperable Address, exposing the same logical components (`address` and `chain`) while relying on [CAIP-350] for all normalization and serialization semantics. All validation, equality, and canonicalization semantics are defined only on the binary form.

Future standards MAY extend these encoding formats (e.g., by adding checksums, additional fields, or readable identifiers for the chain or address fields) as long as the extended form remains trivially and losslessly convertible to the binary _Interoperable Address_ defined in this specification.
...
**Interoperable Address String and JSON**

The _Interoperable Address String_ and _Interoperable Address JSON_ representations are:

| Key | Value |
| :--- | :--- |
| **Interoperable Address String** | `0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045@eip155:1` |
| **Interoperable Address JSON** | `{ "address": "0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045", "chain": "eip155:1" }` |
...
| Key | Value |
| :--- | :--- |
| **Interoperable Address String** | `MJKqp326RZCHnAAbew9MDdui3iCKWco7fsK9sVuZTX2@solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpKuc147dw2N9d` |
| **Interoperable Address JSON** | `{ "address": "MJKqp326RZCHnAAbew9MDdui3iCKWco7fsK9sVuZTX2", "chain": "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpKuc147dw2N9d" }` |
...
| Key | Value |
| :--- | :--- |
| **Interoperable Address String** | `0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045@eip155` |
| **Interoperable Address JSON** | `{ "address": "0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045", "chain": "eip155" }` |
...
| Key | Value |
| :--- | :--- |
| **Interoperable Address String** | `@solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpKuc147dw2N9d` |
| **Interoperable Address JSON** | `{ "chain": "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpKuc147dw2N9d" }` |
...
- MAY introduce stricter syntactic rules for the _Interoperable Address String_ and _Interoperable Address JSON_ (thereby narrowing the set of valid encodings), but MUST NOT introduce new syntactic forms that enlarge the grammar.
- MUST allow for the appropriate binary data to be derived from either encoding format to allow for the construction of an _Interoperable Address_ encoded as defined in this specification
